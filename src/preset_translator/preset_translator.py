...
class TranslationConfig:
    ...
    def _load_languages(self):
        ...
        try:
            with open(LANGUAGES_FILE, 'r', encoding='utf-8') as f:
                ...
                # Ensure Portuguese is correctly mapped
                if "Português" not in languages:
                    languages["Português"] = "pt"
                if "pt" not in languages.values():
            ...
            print("Error: Language file not found or malformed. Using defaults.")
    ...
    def _load_translations(self):
        ...
        try:
            ...
            print("Error: Translation file not found. Using English.")
            return {"en": {}}
...
class TranslationEngine:
    ...
    def translate_text(self, text, target_lang, translate_angle=False, precise_mode=False):
        ...
        # Join chunks and apply final formatting
...
class TranslatorApp:
    ...
    def setup_ui(self):
        ...
        lang_names = list(self.config.languages.keys())
        lang_codes = list(self.config.languages.values())
        if current_lang in lang_codes:
            self.lang_combobox.set(lang_names[lang_codes.index(current_lang)])
        else:
            self.lang_combobox.set(lang_names[0])
        ...
        select_button = ttk.Button(button_frame, text=self.config.get_text("select_button"), command=self.select_files)
        ...
        # Translate button
        self.translate_button = ttk.Button(main_frame, text=self.config.get_text("translate_button"), command=self.start_translation)
    ...
    def update_language(self, event=None):
        ...
        if selected_lang in self.config.languages:
            self.config.current_lang = self.config.languages[selected_lang]
            ...
            messagebox.showinfo(
                self.config.get_text("language_updated_title"),
                self.config.get_text("language_updated_message")
            )
        else:
    ...
    def start_translation(self):
        ...
        target_language = self.config.languages.get(self.lang_combobox.get())
        ...
        translation_thread = threading.Thread(
            ...
            args=(selected_files, target_language, translate_angle, precise_mode, save_location)
        )
            ...
            try:
                ...
                if save_location == "custom":
                    save_path = filedialog.asksaveasfilename(
                        ...
                        title=self.config.get_text("save_translated_file")
                    )
